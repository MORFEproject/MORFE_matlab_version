classdef ReducedAssembly < Assembly
    ...
    methods
        ...
        function [T] = tensor_projection_within2ton(self,elementMethodName,projector,SIZE,sumDIMS,mode,varargin)
            % 这个函数是对tensor向projector在2-n模投影，然后组装
            ...
            for j = elementSet
                thisElement = Elements(j).Object;
                index = thisElement.iDOFs;          
                Ve = cellfun(@(x) x(index), projector, 'UniformOutput', false); %#ok<*PFBNS>
                if strcmpi(mode,'ELP') % toggle Element-Level projection
                    [Te, ~] = thisElement.(elementMethodName)(Ve,sumDIMS,inputs{:});
                    T(index) = T(index) + elementWeights(j) *squeeze(Te);
                else
                    [Te, ~] = thisElement.(elementMethodName)(inputs{:});
                    ...
                    for perm_idx = 1:size(all_permutations, 1)
                        T(index) = T(index) + elementWeights(j) * double(ttv(Te, Ve, all_permutations(perm_idx,:)));
                    end
                end
            end
        end
