...
RB1 = [V0(:,1)]; % reduced basis with VM_1 and MD_11
RB2 = [V0(:,2)]; % reduced basis with VM_1 and MD_11
RB3 = [V0(:,3)]; % reduced basis with VM_1 and MD_11
...
projector2 = {RB1,RB2};
projector3 = {RB1,RB2,RB3};
mode = 'ELP';
tic
f3r = RBeamAssembly.tensor_projection_within2ton('T2_project_optimized',projector2,size(M,1), [2 3], mode);
fprintf(' f3r... %.2f s (%.1f elem/s)\n',toc,myMesh.nElements/toc)

mode = 'standard';
tic
f3rstandard = RBeamAssembly.tensor_projection_within2ton('T2_project',projector2,size(M,1), [2 3], mode);
fprintf(' f3r... %.2f s (%.1f elem/s)\n',toc,myMesh.nElements/toc)

norm(f3rstandard-f3r)/norm(f3r)

% mode = 'ELP';
% tic
% f4r = RBeamAssembly.tensor_projection_within2ton('T3_project',projector3,size(M,1), [2 3 4], mode);
% fprintf(' f4r... %.2f s (%.1f elem/s)\n',toc,myMesh.nElements/toc)

mode = 'ELP';
tic
f4r = RBeamAssembly.tensor_projection_within2ton('T3_project_optimized',projector3,size(M,1), [2 3 4], mode);
fprintf(' f4r... %.2f s (%.1f elem/s)\n',toc,myMesh.nElements/toc)

mode = 'standard';
tic
f4rstandard = RBeamAssembly.tensor_projection_within2ton('T3_project',projector3,size(M,1), [2 3 4], mode);
fprintf(' f4r... %.2f s (%.1f elem/s)\n',toc,myMesh.nElements/toc)

norm(f4rstandard-f4r)/norm(f4r)
